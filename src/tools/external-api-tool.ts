...
export function scaffoldExternalApi(opts: ExternalApiConfig): object {
  ...
  files[`${name}/api/client.php`] = generateApiClient(
    name,
    Name,
    opts.base_url,
    opts.endpoints,
  ...
  files[`${name}/api/request.php`] = generateApiRequest(name, Name, options);

  if (options.use_auth) {
    files[`${name}/api/auth.php`] = generateApiAuth(name, Name, options);
  }

  if (options.use_rate_limit) {
    files[`${name}/api/rate_limiter.php`] = generateRateLimiter(name, Name, options);
  }

  if (options.use_cache) {
    files[`${name}/api/cache.php`] = generateApiCache(name, Name, options);
  }

  files[`${name}/api/hooks.php`] = generateApiHooks(name, Name, opts.endpoints, options);
...
function generateApiRequest(name: string, Name: string, _options: any): string {
  return `<?php
...
function generateApiAuth(name: string, Name: string, options: any): string {
...
function generateRateLimiter(name: string, Name: string, options: any): string {
  return `<?php
...
function generateApiCache(name: string, Name: string, _options: any): string {
  return `<?php
...
function generateApiHooks(
  ...
  return `<?php
// InstantCMS 2. ${name}/api/hooks.php
