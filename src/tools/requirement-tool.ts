...
import { databaseSchema } from "../data/database-schema.js";
...
export function analyzeRequirement(requirement: string): RequirementAnalysis {
  ...
  if (lower.includes('админ') || lower.includes('панель') || lower.includes('backend') || lower.includes('admin')) {
  ...
  if (lower.includes('событи') || lower.includes('хуки') || lower.includes('hook') || lower.includes('интеграц')) {
  ...
  if (lower.includes('загруз') || lower.includes('upload') || lower.includes('файл') || lower.includes('image')) {
  ...
  if (lower.includes('REST') || lower.includes('api') || lower.includes('ajax') || lower.includes('json')) {
  ...
  if (lower.includes('каталог') || lower.includes('товар') || lower.includes('product') || lower.includes('catalog')) {
    suggestedTables.push({
      name: 'products',
      fields: ['id', 'title', 'description', 'price', 'category_id', 'user_id', 'date_pub', 'is_pub']
    });
    suggestedTables.push({
      name: 'categories',
      fields: ['id', 'parent_id', 'title', 'slug', 'ordering']
  ...
  if (lower.includes('блог') || lower.includes('статья') || lower.includes('post')) {
    suggestedTables.push({
      name: 'posts',
      fields: ['id', 'title', 'slug', 'content', 'user_id', 'date_pub', 'is_pub', 'views', 'rating']
    });
