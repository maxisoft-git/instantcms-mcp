...
export function scaffoldOAuth(opts: OAuthConfig): object {
  ...
  files[`${name}/oauth/client.php`] = generateOAuthClient(name, Name, opts.providers, options);
  files[`${name}/oauth/provider.php`] = generateOAuthProvider(name, Name, opts.providers, options);
  files[`${name}/oauth/callback.php`] = generateOAuthCallback(name, Name, options);

  if (options.store_tokens_in_db) {
    files[`${name}/oauth/storage.php`] = generateOAuthStorage(name, Name, options);
  }

  files[`${name}/oauth/hooks.php`] = generateOAuthHooks(name, Name, opts.providers, options);
...
function generateOAuthCallback(name: string, Name: string, options: any): string {
  return `<?php
...
function generateOAuthStorage(name: string, Name: string, _options: any): string {
  return `<?php
...
function generateOAuthHooks(
  ...
  return `<?php
// InstantCMS 2. ${name}/oauth/hooks.php
